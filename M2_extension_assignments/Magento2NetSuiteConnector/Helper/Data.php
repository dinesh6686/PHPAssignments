...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function createCeligoSalesOrderTable($installer)
    {
        ...
        if (!$installer->getConnection()->isTableExists($installer->getTable('celigo_sales_order'))) {
            $config = $installer->getConnection('sales')->getConfig();
            $dbName = $config['dbname'];
            $sales_order_table = $installer->getTable('sales_order');
            $tablePrefix = str_replace('sales_order', '', $sales_order_table);
            $celigo_sales_order = $tablePrefix . 'celigo_sales_order';
            $table = $installer->getConnection()->newTable(
                            ...
                            'parent_id', \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER, null, ['unsigned' => true], 'Parent Id'
                    )->addColumn(
                            'is_exported_to_io', \Magento\Framework\DB\Ddl\Table::TYPE_SMALLINT, 1, ['unsigned' => true, 'default' => 0], 'Is Exported To IO'
                    )->addColumn(
                            ...
                            $installer->getIdxName($celigo_sales_order, ['parent_id']), ['parent_id']
                    )->addForeignKey(
                            $installer->getFkName($celigo_sales_order, 'parent_id', $sales_order_table, 'entity_id'), 'parent_id', $dbName . '.' . $sales_order_table, 'entity_id', \Magento\Framework\DB\Ddl\Table::ACTION_SET_NULL
