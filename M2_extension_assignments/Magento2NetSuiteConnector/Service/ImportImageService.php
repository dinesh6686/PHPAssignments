...
class ImportImageService
{
        ...
        if ($result) {
            ...
            try {
                if (!$existingData) {
                    if (version_compare($this->productMetadata->getVersion(), '2.2.2', '<')) {
                        $this->addImage($product, $newFileName, $imageType, false, $disabled);
                    } else {
                        $this->imageprocessor->addImage($product, $newFileName, $imageType, false, $disabled);
                    }
                } else {
            ...
            if (!empty($id)) {
                try {
                    ...
                    throw new LocalizedException(
                        __(
                            'The SKU ' . $product->getSku() . ' unable to unlink existing image in Magento. Please retry.'
                        )
                    );
                }
            }
    ...
    private function findExistingImageData($existingMediaGalleryEntries, $fileName)
    {
        ...
        $exactFileName = $fileNameArray[0];
        foreach ($existingMediaGalleryEntries as $key => $entry) {
            $isMatchingString = strpos($entry->getFile(), $exactFileName);
            if ($isMatchingString === false) {
                continue;
            }
            if ($isMatchingString >= 0) {
                $existingData = $entry;
                break;
            }
        }
